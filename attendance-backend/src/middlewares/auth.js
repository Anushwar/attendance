...
exports.studentAuth = async (req, res, next) => {
  const { 'x-user-id': id, 'x-user-password': password } = req.headers;
  if (id === undefined || password === undefined) {
    unauthorizedResponse(res, createPermissionError('student_credentials_missing', 'Student not Authorized'));
    return;
  }
  try {
    const user = await getStudentDetails(id);
    req.user = user;
    if (user.password !== password) {
      unauthorizedResponse(res, createPermissionError('student_password_mismatch', 'Student password does not match'));
      return;
    }
    next();
  } catch (e) {
